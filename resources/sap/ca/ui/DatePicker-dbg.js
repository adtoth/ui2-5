...
sap.ca.ui.DatePicker.prototype._setUpDateType = function ()
{
	...
	if (needsDateType)
	{
	    ...
	    var locale = sap.m.getLocaleData();
	    
	    var datePattern = locale.getDatePattern("short");
	    
	    this._dateType.setFormatOptions({pattern: datePattern});
	}
...
sap.ca.ui.DatePicker.prototype.fireChange = function( bInvalidValue ) 
{
	if (!this._dateType)
	{
	    this._setUpDateType();
...
sap.ca.ui.DatePicker.prototype._calendarIconPress = function() 
{
	if (this.getEditable())
	{
		...
		if (tempDate)
		{
	    	...
	    	this._oCalendar.setCurrentDate(tempDate);
	    	this._oCalendar.toggleDatesSelection([tempDate],true);
		}
