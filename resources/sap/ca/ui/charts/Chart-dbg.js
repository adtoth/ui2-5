 ...
 * <li>Events
 * <ul>
 * <li>{@link sap.ca.ui.charts.Chart#event:selectDataPoint selectDataPoint} : fnListenerFunction or [fnListenerFunction, oListenerObject] or [oData, fnListenerFunction, oListenerObject]</li></ul>
 * </li>
 * </ul> 
...
sap.ca.ui.charts.Chart.M_EVENTS = {'selectDataPoint':'selectDataPoint'};
...
sap.ca.ui.charts.Chart.prototype.setMinShapeSize = function (sMinShapeSize) {
    var iSize = parseInt(sMinShapeSize);
    if (iSize < 48) {
        jQuery.sap.warning("MinShapeSize must at least be 48px to be clickable on mobile")
    }
        ...
        for (var p = 0; p < oData.length; p++) {
            for (var q = 0; q < oData[p].data.length; q++) {
                ...
                if (oContext.type === 'Measure') {
                    ...
                    if (this._oSelectedDataList.getGroupIndexFor(sAxisName) === -1) {
                        this._oSelectedDataList.addGroup({
                            key:sAxisName,
                            title:sAxisName
                        });
                    }
                    ...
                    if (this._oSelectedDataList.getItemIndexFor(sAxisName, sKey) === -1) {
                        this._oSelectedDataList.addItemTo(sAxisName, [
                            {
                                key:sKey,
                                color:oColor,
                                label:sMeasureName,
            ...
            for (var p = 0; p < oData.length; p++) {
                for (var q = 0; q < oData[p].data.length; q++) {
                    ...
                    if (oContext.type === 'Measure') {
                        ...
                        this._oSelectedDataList.removeItemFrom(sAxisName, sKey);

                        if (this._oSelectedDataList.getItemIndexFor(sAxisName, sKey) !== -1) {
                            this._oSelectedDataList.removeItemFrom(sAxisName, sKey);
                        }
                        if (this._oSelectedDataList.getNumberOfItemsFor(sAxisName) === 0) {
                            this._oSelectedDataList.removeGroup(sAxisName);
                        }
                    }
                }
            }
...
sap.ca.ui.charts.Chart.prototype.onAfterRendering = function () {
    ...
    if (jQuery.device.is.desktop) {
        this.sResizeListenerId = sap.ui.core.ResizeHandler.register(this.$().parents().find('section').first()[0], jQuery.proxy(this._onResize, this));
    } else {
    ...
    if (vizDataset && !vizDataset._emptyDataset && vizDataset._analysisAxis.length > 0) {
        ...
        if (jQuery.sap.endsWith(initialWidth, "%") || initialWidth == "auto") {
            ...
            if (this.getParent() != null && this.getParent().getDomRef() != null) {
                var $DomRef = this.$();
                var $ParentSection = $DomRef.parents('section').first();
...
sap.ca.ui.charts.Chart.prototype.setPopoverFooter = function (oFooter) {
    if (oFooter && oFooter instanceof sap.m.Bar) {
        if (!jQuery.device.is.phone) {
