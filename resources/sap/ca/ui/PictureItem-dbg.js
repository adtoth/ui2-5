...
sap.ui.core.Control.extend("sap.ca.ui.PictureItem", { metadata : {
	...
	properties : {
		...
		"width" : {type : "sap.ui.core.CSSSize", group : "Data", defaultValue : '62px'},
		"height" : {type : "sap.ui.core.CSSSize", group : "Data", defaultValue : '62px'}
	},
	events : {
		"loaded" : {}
	}
 ...
 * @param file {File}
 */
sap.ca.ui.PictureItem.prototype.setFile = function(file) {

	var that = this;
	
	this._internalFile = file;

    var maxW = Math.min(800, jQuery(window).width());
    var maxH = Math.min(800, jQuery(window).height());
    var canvasSize = Math.max(maxW, maxH);
	// resize image
    ...
    if (domRef) {
        ...
        if (domRef.width == 0 || domRef.height == 0) {
            ...
            if (this._iRenderCount <= 5) {
                ...
                scaledWidth = this.getHeight(); // best default option
                this._iRenderCount = 0;
            }
        }
        else {
        ...
        this._oImage.$().width(scaledWidth);
        this._oImage.$().height(scaledHeight);
    }
...
sap.ca.ui.PictureItem.prototype.onAfterRendering = function() {
    ...
    $image.css({"margin-left":((parseInt(this.getWidth()) - $image.width()) / 2) + "px",
                "margin-top": ((parseInt(this.getHeight()) - $image.height()) / 2) + "px"});
