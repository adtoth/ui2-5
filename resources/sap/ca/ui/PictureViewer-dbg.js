...
/*
    var pictureTileIndexToSelect, numberOfPictures;
    numberOfPictures = this.getTiles().length;

    if (typeof iIndex != "number") {
        pictureTileIndexToSelect = 0;
    } else if (iIndex < 0) {
        pictureTileIndexToSelect = 0;
    } else if (iIndex >= numberOfPictures) {
        pictureTileIndexToSelect = numberOfPictures - 1;
    } else {
        pictureTileIndexToSelect = iIndex;
    }
*/  
    if (this._bRendered) {
    	this.addStyleClass("sapCaPWRendering");
...
sap.ca.ui.PictureViewer.prototype._update = function(bAnimated) {
	...
	this.scrollIntoView(this._selectedIndex || 0, bAnimated);
